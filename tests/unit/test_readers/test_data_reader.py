import pytest
from cdisc_rules_engine.readers.data_reader import DataReader


ADAM_DOMAINS = ["ADAE", "ADEF", "ADSL", "ADTTE"]
SDTM_DOMAINS = ["AE", "DM", "EX", "LB", "SUPPDM", "TA", "TD", "TE", "TI", "TS", "TV", "XP"]


@pytest.fixture
def clinical_data_directory(resources_directory):
    """Get the clinical data directory."""
    return resources_directory / "clinical_data"


def get_all_data_files(clinical_data_directory):
    """Get all clinical data files."""
    xpt_files = list((clinical_data_directory / "xpt").glob("*.xpt"))
    sas_files = list((clinical_data_directory / "sas7bdat").glob("*.sas7bdat"))
    return sorted(xpt_files + sas_files)


def test_metadata_extraction_adam(clinical_data_directory):
    """Test metadata extraction for ADaM files."""
    adam_file = clinical_data_directory / "xpt" / "adsl.xpt"
    if not adam_file.exists():
        pytest.skip("ADaM test file not found")

    reader = DataReader(str(adam_file))

    assert reader.metadata.domain == "ADSL"
    assert reader.metadata.standard_type == "ADaM"
    assert reader.metadata.file_format == "xpt"


def test_metadata_extraction_sdtm(clinical_data_directory):
    """Test metadata extraction for SDTM files."""
    sdtm_file = clinical_data_directory / "xpt" / "dm.xpt"
    if not sdtm_file.exists():
        pytest.skip("SDTM test file not found")

    reader = DataReader(str(sdtm_file))

    assert reader.metadata.domain == "DM"
    assert reader.metadata.standard_type == "SDTM"
    assert reader.metadata.file_format == "xpt"


def test_all_files_readable(clinical_data_directory):
    """Test that all clinical data files can be read."""
    files = get_all_data_files(clinical_data_directory)

    assert len(files) > 0, "No clinical data files found"

    for file_path in files:
        try:
            reader = DataReader(str(file_path))
            result = reader.read()

            assert isinstance(result, dict)
            assert "metadata" in result
            assert "data" in result
            assert "variables" in result
            assert len(result["data"]) > 0, f"No data in {file_path.name}"

        except Exception as e:
            pytest.fail(f"Failed to read {file_path.name}: {str(e)}")


def test_standard_type_classification(clinical_data_directory):
    """Test that files are correctly classified as ADaM or SDTM."""
    files = get_all_data_files(clinical_data_directory)

    assert len(files) > 0, "No clinical data files found"

    for file_path in files:
        reader = DataReader(str(file_path))
        domain = reader.metadata.domain

        if domain in ADAM_DOMAINS:
            assert reader.metadata.standard_type == "ADaM", f"{domain} should be classified as ADaM"
        elif domain in SDTM_DOMAINS:
            assert reader.metadata.standard_type == "SDTM", f"{domain} should be classified as SDTM"
        else:
            raise ValueError(f"Unknown domain {domain} in file {file_path.name}")


def test_file_format_support(clinical_data_directory):
    """Test reading both XPT and SAS7BDAT formats."""
    xpt_file = clinical_data_directory / "xpt" / "ae.xpt"
    if xpt_file.exists():
        reader = DataReader(str(xpt_file))
        result = reader.read()
        assert reader.metadata.file_format == "xpt"
        assert len(result["data"]) > 0

    sas_file = clinical_data_directory / "sas7bdat" / "ae.sas7bdat"
    if sas_file.exists():
        reader = DataReader(str(sas_file))
        result = reader.read()
        assert reader.metadata.file_format == "sas7bdat"
        assert len(result["data"]) > 0


def test_variable_extraction(clinical_data_directory):
    """Test variable information extraction."""
    test_file = clinical_data_directory / "xpt" / "dm.xpt"
    if not test_file.exists():
        pytest.skip("Test file not found")

    reader = DataReader(str(test_file))
    result = reader.read()

    variables = result["variables"]
    assert len(variables) > 0

    for var in variables:
        assert "name" in var
        assert "order" in var
        assert "type" in var
        assert var["type"] in ["Num", "Char", "Unknown"]


def test_with_variable_metadata(clinical_data_directory):
    """Test reading with variable metadata."""
    data_file = clinical_data_directory / "xpt" / "ae.xpt"
    metadata_dir = clinical_data_directory / "variable_metadata"

    if not data_file.exists() or not metadata_dir.exists():
        pytest.skip("Required files not found")

    reader = DataReader(str(data_file), str(metadata_dir))
    result = reader.read()

    assert "expected_variables" in result
    assert "missing_variables" in result
    assert "unexpected_variables" in result
