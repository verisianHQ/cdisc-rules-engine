...
class LocalDataService(BaseDataService):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(LocalDataService, self).__init__(cache_service, reader_factory, config, **kwargs)
    ...
    def has_all_files(self, prefix: str, file_names: List[str]) -> bool:
        files = [f.lower() for f in os.listdir(prefix) if os.path.isfile(os.path.join(prefix, f))]
    ...
    def get_raw_dataset_metadata(self, dataset_name: str, **kwargs) -> SDTMDatasetMetadata:
        ...
        """
        file_metadata, contents_metadata = self.__get_dataset_metadata(dataset_name, **kwargs)
    ...
    def get_variables_metadata(self, dataset_name: str, datasets: list, **params) -> DatasetInterface:
        """
        ...
        return self.dataset_implementation.from_dict(metadata_to_return.to_representation())
    ...
    def read_metadata(self, file_path: str, datasets: Optional[Iterable[SDTMDatasetMetadata]] = None) -> dict:
    ...
    def __get_dataset_metadata(self, dataset_name: str, **kwargs) -> Tuple[dict, dict]:
        ...
        if size_unit:  # convert file size from bytes to desired unit if needed
            file_metadata["file_size"] = convert_file_size(file_metadata["file_size"], size_unit)
        return file_metadata, metadata["contents_metadata"]

    def to_parquet(self, file_path: str) -> str:
        reader = self._reader_factory.get_service(extract_file_name_from_path_string(file_path).split(".")[1].upper())
        return reader.to_parquet(file_path)

    def get_datasets(self) -> List[dict]:
        datasets = [self.get_raw_dataset_metadata(dataset_name=dataset_path) for dataset_path in self.dataset_paths]
        return datasets
