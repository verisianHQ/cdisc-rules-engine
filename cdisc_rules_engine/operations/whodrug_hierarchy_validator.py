...
class WhodrugHierarchyValidator(BaseOperation):
    def _execute_operation(self):
        # get metadata
        whodrug_path = self.params.external_dictionaries.get_dictionary_path(DictionaryTypes.WHODRUG.value)
        if not whodrug_path:
        ...
        code_strings = [f"{self.params.domain}{variable}" for variable in code_variables]
        valid_code_hierarchies = self.get_code_hierarchies(terms)
        column = str(uuid4()) + "_codes"
        self.params.dataframe[column] = self.params.dataframe[code_strings].agg("/".join, axis=1)
    ...
    def get_code_hierarchies(self, term_map: dict) -> Set[str]:
        valid_codes = set()
        for atc_class in term_map.get(WhodrugRecordTypes.ATC_CLASSIFICATION.value, {}).values():
            atc_text = term_map.get(WhodrugRecordTypes.ATC_TEXT.value, {}).get(atc_class.code)
            if atc_text:
                drug_dict = term_map.get(WhodrugRecordTypes.DRUG_DICT.value, {}).get(atc_class.get_parent_identifier())
                if drug_dict:
                    valid_codes.add(f"{drug_dict.drugName}/{atc_text.text}/{atc_class.code}")
