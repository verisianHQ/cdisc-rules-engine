...
class VariableValueCount(BaseOperation):
    ...
    async def _get_all_study_variable_value_counts(self) -> dict:
        ...
        """
        datasets_with_unique_domains = list({dataset.domain: dataset for dataset in self.params.datasets}.values())
        coroutines = [self._get_dataset_variable_value_count(dataset) for dataset in datasets_with_unique_domains]
    ...
    async def _get_dataset_variable_value_count(self, dataset_metadata: SDTMDatasetMetadata) -> Counter:
        if dataset_metadata.is_split:
            corresponding_datasets = get_corresponding_datasets(self.params.datasets, dataset_metadata)
            ...
            data: DatasetInterface = self.data_service.get_dataset(
                dataset_name=os.path.join(self.params.directory_path, dataset_metadata.filename)
            )
            data = tag_source(data, dataset_metadata)
        target_variable = self.params.original_target.replace("--", dataset_metadata.domain, 1)
