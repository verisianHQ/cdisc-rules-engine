...
class IsInconsistentAcrossDatasetOperator(BaseSqlOperator):
    ...
    def _handle_single_comparator(self, target_column, comparator_column, where_populated=False):
        ...
        def generate_update_query(db_table: str, db_column: str) -> str:
            ...
            if where_populated:
                populated_filter = (
                    f" AND NULLIF(CAST(t2.{target} AS TEXT), '') IS NOT NULL"
                    f" AND NULLIF(CAST(t2.{comp} AS TEXT), '') IS NOT NULL"
                )
                current_row_filter = (
                    f" AND NULLIF(CAST(t1.{target} AS TEXT), '') IS NOT NULL"
                    f" AND NULLIF(CAST(t1.{comp} AS TEXT), '') IS NOT NULL"
                )
            return f"""
                    ...
                    SELECT
                        ...
                        (
                            SELECT COUNT(DISTINCT
                                CASE
                                    WHEN t2.{target} IS NULL THEN 'NULL_VALUE'
                                    ELSE CAST(t2.{target} AS TEXT)
                                END
                            )
                            FROM {db_table} AS t2
                            WHERE (
