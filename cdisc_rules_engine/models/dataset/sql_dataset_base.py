...
class SQLDatasetBase(DatasetInterface, ABC):
    ...
    @abstractmethod
    def _serialize_json(self, data: dict) -> Any:
        """Serialize dict to JSON for database storage."""
        pass
    ...
    @abstractmethod
    def concat(self, other: Union['SQLDatasetBase', List['SQLDatasetBase']], 
               axis=0, **kwargs):
    ...
    @classmethod
    def cartesian_product(cls, left: 'SQLDatasetBase', 
                         right: 'SQLDatasetBase') -> 'SQLDatasetBase':
        ...
        for orig_row in self.iterrows():
            ...
            for var in value_vars:
                if var in row_data:
                    ...
                    insert_values.append((new_dataset_id, row_num, self._serialize_json(new_row)))
                    row_num += 1
