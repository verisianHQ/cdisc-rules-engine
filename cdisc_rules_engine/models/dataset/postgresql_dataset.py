...
from cdisc_rules_engine.config.databases.postgresql_database_config import (
...
class PostgreSQLDataset(SQLDatasetBase):
    ...
    def _insert_records(self, records: List[dict]):
        ...
        values = [(self.dataset_id, idx, Json(record)) for idx, record in enumerate(records)]
    ...
    def concat(
        self,
        other: Union["PostgreSQLDataset", List["PostgreSQLDataset"]],
        axis=0,
        **kwargs,
    ):
    ...
    def merge(self, other: "PostgreSQLDataset", on=None, how="inner", **kwargs):
        ...
        if on:
            if isinstance(on, str):
                on = [on]
            join_conditions = " AND ".join([f"a.data->>'{col}' = b.data->>'{col}'" for col in on])
        else:
            join_conditions = "1=1"
    ...
    def _execute_aggregation(self, groupby_cols: List[str], func_dict: Dict[str, Union[str, List[str]]]):  # noqa: C901
        ...
        select_cols = ", ".join([f"data->>'{col}' as {col}" for col in groupby_cols])
        ...
        return PostgreSQLDataset.from_records(records, connection_pool=self.database_config)
