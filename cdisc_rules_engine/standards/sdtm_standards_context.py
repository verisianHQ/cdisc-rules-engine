...
class SdtmStandardsContext(BaseStandardsContext):
    ...
    def within_rule_scope(
        ...
        is_suitable, reason = self.rule_applies_to_variables(metadata, rule, domain, all_datasets=all_datasets)
        if not is_suitable:
            logger.info(f"is_suitable_for_validation. {reason}, result=False")
            return False, reason
    ...
    def rule_applies_to_variables(
        self,
        dataset_metadata: DatasetMetadata2,
        rule: dict,
        domain: str,
        all_datasets: Optional[List[DatasetMetadata2]] = None,
    ) -> Tuple[bool, str]:
        """
        Check if the variables accessed in the rule exist in the dataset or joined datasets.
        """
        required_vars = self._extract_variables_from_rule(rule)
        dataset_vars = {v.name.upper() for v in dataset_metadata.variables}
        all_datasets = all_datasets or []

        joined_domains, relrec_wildcards = self._get_joined_domains_and_wildcards(rule)
        for var in required_vars:
            normalized_var = var.replace("--", domain) if "--" in var else var
            normalized_var_upper = normalized_var.upper()

            if normalized_var_upper in dataset_vars:
                continue

            if self._variable_exists_in_joined_datasets(
                normalized_var_upper, joined_domains, relrec_wildcards, all_datasets
            ):
                continue

            reason = (
                f"Rule skipped - required variable {normalized_var} "
                f"not found in dataset {dataset_metadata.name} or joined datasets"
            )
            return False, reason
    ...
    @staticmethod
    def _get_joined_domains_and_wildcards(rule: dict) -> Tuple[Set[str], Dict[str, str]]:
        """
        Extracts joined domains and RELREC wildcards from rule definitions.
        """
        joined_domains = set()
        relrec_wildcards = {}

        all_joins = (rule.get("datasets") or []) + (rule.get("Match Datasets") or [])
        for join_item in all_joins:
            j_name = (join_item.get("domain_name") or join_item.get("Name") or join_item.get("name") or "").upper()

            if not j_name:
                continue

            joined_domains.add(j_name)

            wildcard = (
                join_item.get("wildcard")
                or join_item.get("Wildcard")
                or join_item.get("Wildcard Column")
                or join_item.get("wildcard_column")
            )
            if j_name == "RELREC" and wildcard:
                relrec_wildcards[j_name] = wildcard.upper()

        return joined_domains, relrec_wildcards

    def _variable_exists_in_joined_datasets(
        self,
        variable_name: str,
        joined_domains: Set[str],
        relrec_wildcards: Dict[str, str],
        all_datasets: List[DatasetMetadata2],
    ) -> bool:
        """
        Checks if a variable exists in any of the joined datasets.
        """
        target_ref_domain, var_to_check = variable_name.split(".", 1) if "." in variable_name else (None, variable_name)

        search_domains = self._resolve_search_domains(target_ref_domain, joined_domains, relrec_wildcards)

        for j_domain in search_domains:
            if not j_domain:
                continue
            ...
            ref_ds = next(
                (
                    ...
                    if (getattr(ds, "domain", "").upper() == j_domain or ds.name.upper() == j_domain)
                ),
                None,
            )

            if ref_ds and any(v.name.upper() == var_to_check for v in ref_ds.variables):
                return True
    ...
    def _resolve_search_domains(
        self,
        target_ref_domain: Optional[str],
        joined_domains: Set[str],
        relrec_wildcards: Dict[str, str],
    ) -> List[str]:
        """
        Determines which domains to search based on variable prefix and wildcards.
        """
        if target_ref_domain:
            if target_ref_domain == "RELREC" and target_ref_domain in relrec_wildcards:
                return [relrec_wildcards[target_ref_domain]]
            else:
                return [target_ref_domain]
        return list(joined_domains)

    def _extract_variables_from_rule(self, rule: dict) -> Set[str]:
        """
        Recursively extract target variables from rule conditions.
        """
        variables = set()
        ...
        leaf_conditions = (
            conditions.values() if hasattr(conditions, "values") else self._flatten_raw_conditions(conditions)
        )

        for cond in leaf_conditions:
            if not isinstance(cond, dict) or cond.get("operator") in ["exists", "not_exists"]:
                continue

            potential_vars = self._get_potential_vars_from_condition(cond)
    ...
    def _get_potential_vars_from_condition(self, condition: dict) -> List[str]:
        """
        Extracts variable names from a single condition dictionary.
        """
        potential_vars = []
    ...
    def _is_valid_variable(self, var: str) -> bool:
        ignored_names = {
            "variable_name",
            "variable_label",
            "variable_value",
        ...
        return isinstance(var, str) and var not in ignored_names and "$" not in var

    def _flatten_raw_conditions(self, conditions: Any) -> List[dict]:
        """Helper to flatten nested raw dictionary conditions (pre-composite)."""
        ...
        if "all" in conditions:
            ...
            for c in conditions["all"]:
                result.extend(self._flatten_raw_conditions(c))
            return result
        ...
        if "not" in conditions:
            return self._flatten_raw_conditions(conditions["not"])
